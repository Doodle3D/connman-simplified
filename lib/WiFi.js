...
WiFi.prototype.closeHotspot = function(callback) {
  ...
  function onTetheringChange(tethering) {
    debug('closeHotspot/onTetheringChange: ', tethering);
    if (!tethering) {
      _lockHotspotChange = false;
      debug('closeHotspot/onTetheringChange callback');
      if (callback) callback();
      self.removeListener('servicesChanged', onChange);
      self.removeListener('tethering', onTetheringChange);
      callback = null; 
...
WiFi.prototype.openHotspot = function(ssid, passphrase, callback) {
  ...
  function onTetheringChange(tethering) {
    ...
    _lockHotspotChange = false;
    debug('openHotspot/onTetheringChange callback');
    if (callback) callback();
    self.removeListener('tethering', onTetheringChange);
    self.removeListener('servicesChanged',onChange);
    callback = null; 
  ...
  function onChange() {
    debug('openHotspot/onChange');
    debug('properties.tethering: ' + self.properties.tethering);
    _lockHotspotChange = false;
    self.removeListener('tethering', onTetheringChange);
    self.removeListener('servicesChanged',onChange);
    if (callback) callback();
  }  
  // self.once('servicesChanged',onChange); //NOTE: do not add here but later
...
//      } 
  ...
  if (self.properties.tethering) {
    _lockHotspotChange = false;
    debug('openHotspot: first close hotspot...');
    self.closeHotspot(function(err) {
      if (err) {
        debug(err);
        if (callback) callback(err);
      ...
      debug('openHotspot: closed hotspot, now open hotspot...');
      debug('properties.tethering: ' + self.properties.tethering);
      ...
      if (!self.properties.tethering) {
        ...
        var err = new Error('[ERROR] tethering is not yet disabled...');
        debug(err);
        if (callback) callback(err);
      }
    });
