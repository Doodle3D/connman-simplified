...
var _numGetServiceAttempts = 15;
var _timeoutServicesChanged = 5000;
var _numServicesChangedEvents = 2;
var _getServiceAttemptTimeout = 10000;
...
var _lockOpenHotspot = false;
var _lockCloseHotspot = false;
var _lockJoin = false;
...
WiFi.prototype.join = function(ssid, passphrase, callback) {
  ...
  if (_lockJoin) {
    if (callback) callback(new Error('Already joining network'));
    return;
  };
  _lockJoin = true;
  ...
  async.series([
    ...
    function(next) {
      ...
      if (!passphrase || passphrase === '') {
        ...
        passphraseSender = function(service, fields, cb){
          debug('RequestInput');
          if(fields["Passphrase"] != null) {
            cb({"Passphrase": passphrase});
          }
        }
...
WiFi.prototype.closeHotspot = function(callback) {
  ...
  if (_lockCloseHotspot) {
    if (callback) callback(new Error('Already changing hotspot'));
    return;
  };
  _lockCloseHotspot = true;
  ...
  self.iface.disableTethering(function(err, res) {
    ...
    if (err) {
      ...
      //--a reset as fallback option after timeout
      timeout = setTimeout(function() {
        self.removeListener('servicesChanged', onChange);
        _lockCloseHotspot = false;
        if (callback) callback(err);
...
WiFi.prototype.openHotspot = function(ssid, passphrase, callback) {
  ...
  //--skip
  // if (self.properties.tethering) {
  //   if (callback) callback(new Error('Already hotspot, close it first'));
  //   return;
  // }
  ...
  function onTetheringChange(tethering) {
    debug('openHotspot/onTetheringChange: ', tethering);
    _lockOpenHotspot = false;
    clearTimeout(timeout);
  ...
  //--async
  async.series([
    function doClose(next) {
      debug('doClose');
      self.closeHotspot(next);
    },

    function doOpen(next) {
      debug('doOpen');
  ...
  function(err) {
    if (err) {
      debug('[ERROR] async: ' + err);
      if (callback) callback(err);
      return;
    };
    debug('async done, waiting for callback onChange/onTetheringChange');
    // if (callback) callback(err); //--note: happens on event
