...
WiFi.prototype.join = function(ssid,passphrase,callback) {
  ...
  async.series([
    ...
    function switchService(next) {
      setService(targetService.name,targetServiceProperties);
      next();
    },
...
function setService(serviceName,callback) {
  ...
  _connman.getService(serviceName,function(err,service,serviceProperties) {
    if(err) {
      if(callback) callback(err); 
      return;
    }
    _service = service;
    _serviceProperties = _self.parseService(serviceProperties);
    ...
    _self.emit('serviceChanged',_service,_serviceProperties);
    logStatus();
  });
}
function clearService() {
  ...
  var type;
  // reset properties
  for(type in _serviceProperties) {
...
function updateCurrentService() {
  for(var serviceName in _networks) {
    var serviceProperties = _networks[serviceName];
    if(serviceProperties.state != 'idle') {
      setService(serviceName);
      return;
    }
  }
  clearService(); // no active network found, clear current service
}
