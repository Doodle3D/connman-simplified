...
function onServicePropertyChanged(type, value) {
  ...
  if(type == 'name') {
    this.emit('ssid',value);
    this.emit('connectionPropertyChanged','ssid',value);
  }
}
...
Base.prototype._clearService = function() {
  ...
  debug('clearService');
  self.service.removeAllListeners();
  self.service = null;
  var type;
  // reset properties
  for(type in self.serviceProperties) {
    self.serviceProperties[type] = '';
  }
  self.serviceProperties.state = 'idle';
  for(type in self.serviceProperties) {
    self.emit(type,self.serviceProperties[type]);
  }
  self.emit('serviceChanged',self.service,self.serviceProperties);
};
//function setServices(services,log) {
Base.prototype._setServices = function(services,log) {
  ...
  for (var key in this.services) {
      servicesArr.push(this.services[key]);
  }  
